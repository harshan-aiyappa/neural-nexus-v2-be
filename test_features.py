...
async def test_features():
    ...
    async with httpx.AsyncClient() as client:
        try:
            # 1. Test /folders (GET)
            folders_resp = await client.get("http://10.10.20.122:8000/folders")
            ...
            create_resp = await client.post("http://10.10.20.122:8000/folders", json=new_folder)
            ...
            extract_resp = await client.post("http://10.10.20.122:8000/extract", json=extract_req)
            ...
            if created_folder_id:
                delete_resp = await client.delete(f"http://10.10.20.122:8000/folders/{created_folder_id}")
